...
def main():
    ...
    delta_y = cv2.filter2D(first_grad_img, cv2.CV_32F, kernel)
    kernel = np.array([
        [1, 0, -1],
    ])
    delta_x = cv2.filter2D(first_grad_img, cv2.CV_32F, kernel)
    delta_norm = np.sqrt(delta_x * delta_x + delta_y * delta_y)
    coords = np.where(delta_norm > 50)
    tracking_pts_nx2 = np.vstack((coords[1], coords[0])).T
    ...
    for info in associate[1:]:
        ...
        for p1, p2 in zip(valid_tracking_pts_nx2[mask].astype(int), valid_pixel_now.astype(int)):
            cv2.circle(img_show, tuple(p1), 4, (0, 255, 0), 2)
            cv2.circle(img_show, tuple([p2[0], p2[1] + grad_img.shape[0]]), 4, (0, 255, 0), 2)
